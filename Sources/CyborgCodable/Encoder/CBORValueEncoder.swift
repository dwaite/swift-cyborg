...
// because of the encoder model, vended encoding containers have no scope. So we wind up deferring encoding in some cases to those containers, which get stuck directly into our object model
...
public class ActiveCBOREncoder: Encoder {
    ...
    init(boxing: CBORBoxing, subKey: CodingKey) {
        self.boxing = boxing
        self.boxing.codingPath = self.boxing.codingPath + [subKey]
    }
